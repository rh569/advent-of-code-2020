...
type NestedExpression struct {
	n int      // level of nesting
	e []string // expression as an array of strings (numbers, +, *)
}

type Sum struct {
	s []NestedExpression // the sum
	n int                // the highest term
}

// Returns an a array of 'sums' each being an []NestedExpression
func parseInput(input string) []Sum {
	lines := strings.Split(input, "\n")
	sums := []Sum{}

	for _, l := range lines {
		sums = append(sums, parseSum(l))
	}

	return sums
}

func parseSum(sumStr string) Sum {
	// Ensure spaces are removed
	...
	// Thankfully all input numbers are single digit
	for _, r := range sumStr {

		if r == '(' || r == ')' {
			// Store the previous expression if it has terms
			if len(expression.e) > 0 {
				sum.s = append(sum.s, expression)
			}
			...
			expression = NestedExpression{n: currentN}
		} else {
			expression.e = append(expression.e, string(r))
		}
	...
	// Store the last expression if it has terms
	if len(expression.e) > 0 {
		sum.s = append(sum.s, expression)
	}
...
var MathHomeworkInput = `6 * (2 + 4 * (3 + 7 * 8 + 4) * 8 + 3 + (9 + 8 + 4 + 9)) * 8 + ((2 * 5 * 9 + 7 + 4 * 3) + (8 + 3 + 7) + (7 + 2) * 8 * 8 + 9)
