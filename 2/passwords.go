...
// Split input into policies and passwords
// Compare each policy against its password
...
func main() {
	...
	splitPasswords := splitInput(input)
	numberValid := 0
	numberActuallyValid := 0

	for _, password := range splitPasswords {
		if isValid(password) {
			numberValid++
		}

		if isActuallyValid(password) {
			numberActuallyValid++
		}
	}

	fmt.Println("Part 1:")
	fmt.Printf("Found %v valid passwords in input\n", numberValid)

	fmt.Println("Part 2:")
	fmt.Printf("Found %v actually valid passwords in input\n", numberActuallyValid)
}

// Splits strings in input into 4 parts:
// start of range, end of range, applicable letter, password
func splitInput(input []string) [][]string {
	splitInput := [][]string{}

	for _, password := range input {
		separablePassword := strings.Replace(password, "-", " ", 1)
		separablePassword = strings.Replace(separablePassword, ":", "", 1)

		splitPassword := strings.Split(separablePassword, " ")
		splitInput = append(splitInput, splitPassword)
	}

	return splitInput
}

// given a properly split password [start, end, letter, pass]
// return true if pass matches criteria
func isValid(splitPassword []string) bool {
	var wantedLetters string
	testRune := []rune(splitPassword[2])[0]

	for _, letter := range splitPassword[3] {
		if letter == testRune {
			wantedLetters += string(letter)
		}
	}

	lettersInPass := len(wantedLetters)
	start, _ := strconv.ParseInt(splitPassword[0], 10, 0)
	end, _ := strconv.ParseInt(splitPassword[1], 10, 0)

	return int64(lettersInPass) >= start && int64(lettersInPass) <= end
}

// returns true if the specified letter exits in either
// the first pos specified, or the second but not both
func isActuallyValid(splitPassword []string) bool {
	testRune := []rune(splitPassword[2])[0]

	numberMatchingRunes := 0
	passwordRunes := []rune(splitPassword[3])

	pos1, _ := strconv.ParseInt(splitPassword[0], 10, 0)
	pos2, _ := strconv.ParseInt(splitPassword[1], 10, 0)

	if passwordRunes[pos1-1] == testRune {
		numberMatchingRunes++
	}

	if passwordRunes[pos2-1] == testRune {
		numberMatchingRunes++
	}

	return numberMatchingRunes == 1
}
